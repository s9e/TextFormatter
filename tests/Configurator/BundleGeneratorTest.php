...
class BundleGeneratorTest extends Test
{
	...
	*/
	...
	{
		$mock = $this->getMock('stdClass', array('serialize'));
		$mock->expects($this->any())
		     ->method('serialize')
		     ->will($this->returnValue('O:8:"stdClass":1:{s:6:"foobar";i:1;}'));

		$this->configurator->bundleGenerator->serializer = array($mock, 'serialize');
	...
	*/
	public function testParserCallback()
	{
		...
		$this->configurator->bundleGenerator->generate('Foo', array('finalizeParser' => array($mock, 'foo')));
	}
	...
	*/
	public function testRendererCallback()
	{
		...
		$this->configurator->bundleGenerator->generate('Foo', array('finalizeRenderer' => array($mock, 'foo')));
	}
	...
	*/
	public function testAutoIncludeFalse()
	{
		...
		$bundle    = $this->configurator->bundleGenerator->generate('Foo', array('autoInclude' => false));
		$className = $rendererGenerator->lastClassName;
		$filepath  = $rendererGenerator->lastFilepath;
	...
	*/
	public function testEvents()
	{
		...
		{
			$php = $this->configurator->bundleGenerator->generate('Foo', array($event => 'trim'));
	...
	*/
	public function testRendererSetup()
	{
		$this->assertContains(
			...
			$this->configurator->bundleGenerator->generate(
				'Foo',
				array('rendererSetup' => '\\foo\\bar\\baz')
			)
		);
	}
