...
class TemplateOptimizerTest extends Test
{
	...
	*/
	...
	/**
	* @testdox Values are inlined in <xsl:if test=".=':)'"><xsl:value-of select="."/></xsl:if>
	*/
	public function testE28E2C14()
	{
		$this->runCase(
			'Values are inlined in <xsl:if test=".=\':)\'"><xsl:value-of select="."/></xsl:if>',
	...
	/**
	* @testdox Values are inlined in <xsl:choose><xsl:when test=".=':)'"><img alt="{.}"/></xsl:when><xsl:when test=".=':('"><img alt="{.}"/></xsl:when><xsl:otherwise><xsl:value-of select="."/></xsl:otherwise></xsl:choose>
	*/
	public function test119F725B()
	{
		$this->runCase(
			'Values are inlined in <xsl:choose><xsl:when test=".=\':)\'"><img alt="{.}"/></xsl:when><xsl:when test=".=\':(\'"><img alt="{.}"/></xsl:when><xsl:otherwise><xsl:value-of select="."/></xsl:otherwise></xsl:choose>',
	...
	/**
	* @testdox Values and expressions are preserved in <xsl:if test=".=':)'"><hr title="{{.}}{@foo}"/></xsl:if>
	*/
	public function test13E10F79()
	{
		$this->runCase(
			'Values and expressions are preserved in <xsl:if test=".=\':)\'"><hr title="{{.}}{@foo}"/></xsl:if>',
