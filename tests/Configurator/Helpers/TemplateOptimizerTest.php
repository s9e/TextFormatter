...
class TemplateOptimizerTest extends Test
{
	...
	*/
	...
	/**
	* @testdox <xsl:if test=".=':)'"><xsl:value-of select="."/></xsl:if> is replaced with <xsl:if test=".=':)'">:)</xsl:if>
	*/
	public function test03686B92()
	{
		$this->runCase(
			'<xsl:if test=".=\':)\'"><xsl:value-of select="."/></xsl:if> is replaced with <xsl:if test=".=\':)\'">:)</xsl:if>',
	...
	/**
	* @testdox <xsl:choose><xsl:when test=".=':)'"><img alt="{.}"/></xsl:when><xsl:when test=".=':('"><img alt="{.}"/></xsl:when><xsl:otherwise><xsl:value-of select="."/></xsl:otherwise></xsl:choose> is replaced with <xsl:choose><xsl:when test=".=':)'"><img alt=":)"/></xsl:when><xsl:when test=".=':('"><img alt=":("/></xsl:when><xsl:otherwise><xsl:value-of select="."/></xsl:otherwise></xsl:choose>
	*/
	public function test6825D250()
	{
		$this->runCase(
			'<xsl:choose><xsl:when test=".=\':)\'"><img alt="{.}"/></xsl:when><xsl:when test=".=\':(\'"><img alt="{.}"/></xsl:when><xsl:otherwise><xsl:value-of select="."/></xsl:otherwise></xsl:choose> is replaced with <xsl:choose><xsl:when test=".=\':)\'"><img alt=":)"/></xsl:when><xsl:when test=".=\':(\'"><img alt=":("/></xsl:when><xsl:otherwise><xsl:value-of select="."/></xsl:otherwise></xsl:choose>',
			'<xsl:choose><xsl:when test=".=\':)\'"><img alt="{.}"/></xsl:when><xsl:when test=".=\':(\'"><img alt="{.}"/></xsl:when><xsl:otherwise><xsl:value-of select="."/></xsl:otherwise></xsl:choose>',
			'<xsl:choose><xsl:when test=".=\':)\'"><img alt=":)"/></xsl:when><xsl:when test=".=\':(\'"><img alt=":("/></xsl:when><xsl:otherwise><xsl:value-of select="."/></xsl:otherwise></xsl:choose>'
		);
	}
