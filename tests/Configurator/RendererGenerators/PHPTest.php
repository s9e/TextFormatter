...
class PHPTest extends Test
{
	...
	*/
	...
	public function getEdgeCases()
	{
		...
		return array(
			...
			array(
				'<r><X foo="FOO">..</X></r>',
				function ($configurator, $test) use ($_this)
				{
					if (!extension_loaded('mbstring'))
					{
						$_this->markTestSkipped('Extension mbstring is required.');
					}
	...
	public function getXPathTests()
	{
		...
		return array(
			...
			array(
				...
				array('$this->xpath = new \\DOMXPath($dom);', 'function getParamAsXPath(')
			),
		);
	}
	...
	*/
	...
	{
		$mock = $this->getMock('stdClass', array('optimize'));
	...
	*/
	public function testHTML($xml, $html, $xhtml, $setup = null, $rendererSetup = null)
	{
		...
		extract($this->configurator->finalize(array('returnParser' => false)));
...
!!!!		',
	...
	*/
	public function testPlugins($pluginName, $original, $expected, array $pluginOptions = array(), $setup = null)
	{
		$this->configurator->rendering->engine = 'PHP';
	...
	*/
	public function testPluginsQuick($pluginName, $original, $expected, array $pluginOptions = array(), $setup = null)
	{
		$this->testPlugins(
			$pluginName,
			$original,
	...
	public function getPluginsTests()
	{
		...
		foreach (glob($pluginsDir . '/*', GLOB_ONLYDIR) as $dirpath)
		{
			...
			$obj = new $className;
			if (is_callable(array($obj, 'getRenderingTests')))
			{
