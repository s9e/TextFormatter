...
abstract class Test extends \PHPUnit_Framework_TestCase
{
	...
	protected function assertJSParsing($original, $expected, $assertMethod = 'assertSame')
	{
		...
		$this->configurator->javascript->exportMethods = array('parse');
	...
	protected function execJS($src, $input)
	{
		...
		if (!isset($exec))
		{
			$interpreters = array(
				'js17' => array('print', ' -U'),
				'd8'   => array('print', ''),
				'node' => array('console.log', '')
			);

			foreach ($interpreters as $interpreter => $_1627720907)
			{
				list($function, $options) = $_1627720907;
