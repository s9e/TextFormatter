...
{
	protected $params=array('IS_GECKO'=>'','IS_IE'=>'','IS_OPERA'=>'','L_CODE'=>'Code','L_CODE_SELECT'=>'[Select]','L_QUOTE'=>'Quote','L_QUOTE_FROM'=>'Quote from','L_SEARCH_ON'=>'on','SCRIPT_URL'=>'','SMILEYS_PATH'=>'');
	protected static $tagBranches=array('ABBR'=>0,'ACRONYM'=>1,'ANCHOR'=>2,'B'=>3,'BDO'=>4,'BLACK'=>5,'BLUE'=>6,'BR'=>7,'br'=>7,'CENTER'=>8,'CODE'=>9,'COLOR'=>10,'E'=>11,'EMAIL'=>12,'FLASH'=>13,'FONT'=>14,'FTP'=>15,'GLOW'=>16,'GREEN'=>17,'HR'=>18,'HTML'=>19,'NOBBC'=>19,'I'=>20,'html:em'=>20,'IMG'=>21,'IURL'=>22,'LEFT'=>23,'LI'=>24,'LIST'=>25,'LTR'=>26,'ME'=>27,'MOVE'=>28,'PRE'=>29,'html:pre'=>29,'QUOTE'=>30,'RED'=>31,'RIGHT'=>32,'RTL'=>33,'S'=>34,'html:del'=>34,'SHADOW'=>35,'SIZE'=>36,'SUB'=>37,'SUP'=>38,'TABLE'=>39,'TD'=>40,'TIME'=>41,'TR'=>42,'TT'=>43,'U'=>44,'URL'=>45,'WHITE'=>46,'e'=>47,'i'=>47,'s'=>47,'html:a'=>48,'html:b'=>49,'html:blockquote'=>50,'html:br'=>51,'html:hr'=>52,'html:i'=>53,'html:img'=>54,'html:ins'=>55,'html:s'=>56,'html:u'=>57,'p'=>58);
	protected static $btEB55FB2E=array('8)'=>0,':\'['=>1,':)'=>2,':))'=>3,':-*'=>4,':-X'=>5,':-['=>6,':-\\'=>7,'::)'=>8,':D'=>9,':P'=>10,':['=>11,':o'=>12,';)'=>13,';D'=>14,'>:D'=>15,'>:['=>16,'???'=>17,'C:-)'=>18,'O0'=>19,'O:-)'=>20,'^-^'=>21);
	protected $xpath;
	...
	private static $static=array('/ABBR'=>'</abbr>','/ACRONYM'=>'</acronym>','/ANCHOR'=>'</span>','/B'=>'</span>','/BDO'=>'</bdo>','/BLACK'=>'</span>','/BLUE'=>'</span>','/CENTER'=>'</div>','/COLOR'=>'</span>','/EMAIL'=>'</a>','/FONT'=>'</span>','/FTP'=>'</a>','/GREEN'=>'</span>','/HTML'=>'','/I'=>'</em>','/IURL'=>'</a>','/LEFT'=>'</div>','/LI'=>'</li>','/LTR'=>'</div>','/ME'=>'</div>','/MOVE'=>'</marquee>','/NOBBC'=>'','/PRE'=>'</pre>','/QUOTE'=>'</blockquote><div class="quotefooter"><div class="botslice_quote"></div></div>','/RED'=>'</span>','/RIGHT'=>'</div>','/RTL'=>'</div>','/S'=>'</del>','/SIZE'=>'</span>','/SUB'=>'</sub>','/SUP'=>'</sup>','/TABLE'=>'</table>','/TD'=>'</td>','/TR'=>'</tr>','/TT'=>'</span>','/U'=>'</span>','/URL'=>'</a>','/WHITE'=>'</span>','/html:a'=>'</a>','/html:b'=>'</b>','/html:blockquote'=>'</blockquote>','/html:del'=>'</del>','/html:em'=>'</em>','/html:i'=>'</i>','/html:ins'=>'</ins>','/html:pre'=>'</pre>','/html:s'=>'</s>','/html:u'=>'</u>','B'=>'<span class="bbc_bold">','BLACK'=>'<span style="color: black;" class="bbc_color">','BLUE'=>'<span style="color: blue;" class="bbc_color">','BR'=>'<br>','CENTER'=>'<div align="center">','GREEN'=>'<span style="color: green;" class="bbc_color">','HR'=>'<hr>','HTML'=>'','I'=>'<em>','LEFT'=>'<div style="text-align: left;">','LI'=>'<li>','LTR'=>'<div dir="ltr">','MOVE'=>'<marquee>','NOBBC'=>'','PRE'=>'<pre>','RED'=>'<span style="color: red;" class="bbc_color">','RIGHT'=>'<div style="text-align: right;">','RTL'=>'<div dir="rtl">','S'=>'<del>','SUB'=>'<sub>','SUP'=>'<sup>','TABLE'=>'<table class="bbc_table">','TD'=>'<td>','TR'=>'<tr>','TT'=>'<span class="bbc_tt">','U'=>'<span class="bbc_u">','WHITE'=>'<span style="color: white;" class="bbc_color">','html:b'=>'<b>','html:blockquote'=>'<blockquote>','html:br'=>'<br>','html:del'=>'<del>','html:em'=>'<em>','html:hr'=>'<hr>','html:i'=>'<i>','html:ins'=>'<ins>','html:pre'=>'<pre>','html:s'=>'<s>','html:u'=>'<u>');
	private static $dynamic=array('ABBR'=>array('(^[^ ]+(?> (?!abbr=)[^=]+="[^"]*")*(?> abbr="([^"]*)")?.*)s','<abbr title="$1">'),'ACRONYM'=>array('(^[^ ]+(?> (?!acronym=)[^=]+="[^"]*")*(?> acronym="([^"]*)")?.*)s','<acronym title="$1">'),'ANCHOR'=>array('(^[^ ]+(?> (?!anchor=)[^=]+="[^"]*")*(?> anchor="([^"]*)")?.*)s','<span id="post_$1">'),'BDO'=>array('(^[^ ]+(?> (?!bdo=)[^=]+="[^"]*")*(?> bdo="([^"]*)")?.*)s','<bdo dir="$1">'),'COLOR'=>array('(^[^ ]+(?> (?!color=)[^=]+="[^"]*")*(?> color="([^"]*)")?.*)s','<span style="color: $1;" class="bbc_color">'),'EMAIL'=>array('(^[^ ]+(?> (?!email=)[^=]+="[^"]*")*(?> email="([^"]*)")?.*)s','<a href="mailto:$1" class="bbc_email">'),'FONT'=>array('(^[^ ]+(?> (?!font=)[^=]+="[^"]*")*(?> font="([^"]*)")?.*)s','<span style="font-family: $1;" class="bbc_font">'),'FTP'=>array('(^[^ ]+(?> (?!ftp=)[^=]+="[^"]*")*(?> ftp="([^"]*)")?.*)s','<a href="$1" class="bbc_ftp new_win" target="_blank">'),'IURL'=>array('(^[^ ]+(?> (?!iurl=)[^=]+="[^"]*")*(?> iurl="([^"]*)")?.*)s','<a href="$1" class="bbc_link">'),'SIZE'=>array('(^[^ ]+(?> (?!size=)[^=]+="[^"]*")*(?> size="([^"]*)")?.*)s','<span style="font-size: $1;" class="bbc_size">'),'URL'=>array('(^[^ ]+(?> (?!url=)[^=]+="[^"]*")*(?> url="([^"]*)")?.*)s','<a href="$1" class="bbc_link" target="_blank">'),'html:a'=>array('(^[^ ]+(?> (?!href=)[^=]+="[^"]*")*( href="[^"]*")?.*)s','<a$1>'),'html:img'=>array('(^[^ ]+(?> (?!(?>alt|height|src|width)=)[^=]+="[^"]*")*( alt="[^"]*")?(?> (?!(?>height|src|width)=)[^=]+="[^"]*")*( height="[^"]*")?(?> (?!(?>src|width)=)[^=]+="[^"]*")*( src="[^"]*")?(?> (?!width=)[^=]+="[^"]*")*( width="[^"]*")?.*)s','<img$1$2$3$4>'));
	private static $attributes;
	private static $quickBranches=array('/CODE'=>0,'/GLOW'=>1,'/LIST'=>2,'CODE'=>3,'E'=>4,'FLASH'=>5,'GLOW'=>6,'IMG'=>7,'LIST'=>8,'ME'=>9,'QUOTE'=>10,'TIME'=>11);
	...
	protected function quick($m)
	{
		...
		{
			if ($id[0] === '!' || $id[0] === '?' || \preg_match('(^/?SHADOW$)', $id))
				throw new \RuntimeException;
			return '';
		}
