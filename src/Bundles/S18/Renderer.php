...
class Renderer extends \s9e\TextFormatter\Renderer
{
	...
	private static $dynamic=['ABBR'=>['(^\\S*(?> (?!abbr=)[^=]+="[^"]*")*(?> abbr="([^"]+)")?.*)s','<abbr title="$1">'],'ACRONYM'=>['(^\\S*(?> (?!acronym=)[^=]+="[^"]*")*(?> acronym="([^"]+)")?.*)s','<acronym title="$1">'],'ANCHOR'=>['(^\\S*(?> (?!anchor=)[^=]+="[^"]*")*(?> anchor="([^"]+)")?.*)s','<span id="post_$1">'],'BDO'=>['(^\\S*(?> (?!bdo=)[^=]+="[^"]*")*(?> bdo="([^"]+)")?.*)s','<bdo dir="$1">'],'COLOR'=>['(^\\S*(?> (?!color=)[^=]+="[^"]*")*(?> color="([^"]+)")?.*)s','<span style="color: $1;" class="bbc_color">'],'EMAIL'=>['(^\\S*(?> (?!email=)[^=]+="[^"]*")*(?> email="([^"]+)")?.*)s','<a href="mailto:$1" class="bbc_email">'],'FONT'=>['(^\\S*(?> (?!font=)[^=]+="[^"]*")*(?> font="([^"]+)")?.*)s','<span style="font-family: $1;" class="bbc_font">'],'FTP'=>['(^\\S*(?> (?!ftp=)[^=]+="[^"]*")*(?> ftp="([^"]+)")?.*)s','<a href="$1" class="bbc_ftp new_win" target="_blank">'],'IURL'=>['(^\\S*(?> (?!iurl=)[^=]+="[^"]*")*(?> iurl="([^"]+)")?.*)s','<a href="$1" class="bbc_link">'],'SIZE'=>['(^\\S*(?> (?!size=)[^=]+="[^"]*")*(?> size="([^"]+)")?.*)s','<span style="font-size: $1;" class="bbc_size">'],'URL'=>['(^\\S*(?> (?!url=)[^=]+="[^"]*")*(?> url="([^"]+)")?.*)s','<a href="$1" class="bbc_link" target="_blank">'],'html:a'=>['(^\\S*(?> (?!href=)[^=]+="[^"]*")*( href="[^"]+")?.*)s','<a$1>'],'html:img'=>['(^\\S*(?> (?!(?>alt|height|src|width)=)[^=]+="[^"]*")*( alt="[^"]+")?(?> (?!(?>alt|height|src|width)=)[^=]+="[^"]*")*( height="[^"]+")?(?> (?!(?>alt|height|src|width)=)[^=]+="[^"]*")*( src="[^"]+")?(?> (?!(?>alt|height|src|width)=)[^=]+="[^"]*")*( width="[^"]+")?.*)s','<img$1$2$3$4>']];
	...
	protected function quick($m)
	{
		...
		if($qb<6)if($qb<3)if($qb===0){$html='';if(!empty($this->params['IS_GECKO'])||!empty($this->params['IS_OPERA']))$html.='</code></pre>';else$html.='</code>';}elseif($qb===1){$html='';if(!empty($this->params['IS_IE']))$html.='</td></tr></table>';else$html.='</span>';}else{$attributes=array_pop(self::$attributes);$html='';if(isset($attributes['type']))$html.='</ul>';else$html.='</ul>';}elseif($qb===3){$html='<div class="codeheader">'.htmlspecialchars($this->params['L_CODE'],0).':';if(isset($attributes['lang']))$html.=' ('.str_replace('&quot;','"',$attributes['lang']).')';$html.=' <a href="#" onclick="return smfSelectText(this);" class="codeoperation">'.htmlspecialchars($this->params['L_CODE_SELECT'],0).'</a></div>';if(!empty($this->params['IS_GECKO'])||!empty($this->params['IS_OPERA']))$html.='<pre style="margin: 0; padding: 0;"><code class="bbc_code">';else$html.='<code class="bbc_code">';}elseif($qb===4){$html='';if(isset(self::$btEB55FB2E[$textContent])){$n=self::$btEB55FB2E[$textContent];if($n<11)if($n<6)if($n<3)if($n===0)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'cool.gif" alt="8)" title="Cool" class="smiley">';elseif($n===1)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'cry.gif" alt=":\'[" title="Cry" class="smiley">';else$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'smiley.gif" alt=":)" title="Smiley" class="smiley">';elseif($n===3)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'laugh.gif" alt=":))" title="Laugh" class="smiley">';elseif($n===4)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'kiss.gif" alt=":-*" title="Kiss" class="smiley">';else$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'lipsrsealed.gif" alt=":-X" title="Lips Sealed" class="smiley">';elseif($n<9)if($n===6)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'embarrassed.gif" alt=":-[" title="Embarrassed" class="smiley">';elseif($n===7)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'undecided.gif" alt=":-\\" title="Undecided" class="smiley">';else$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'rolleyes.gif" alt="::)" title="Roll Eyes" class="smiley">';elseif($n===9)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'cheesy.gif" alt=":D" title="Cheesy" class="smiley">';else$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'tongue.gif" alt=":P" title="Tongue" class="smiley">';elseif($n<17)if($n<14)if($n===11)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'sad.gif" alt=":[" title="Sad" class="smiley">';elseif($n===12)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'shocked.gif" alt=":o" title="Shocked" class="smiley">';else$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'wink.gif" alt=";)" title="Wink" class="smiley">';elseif($n===14)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'grin.gif" alt=";D" title="Grin" class="smiley">';elseif($n===15)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'evil.gif" alt="&gt;:D" title="Evil" class="smiley">';else$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'angry.gif" alt="&gt;:[" title="Angry" class="smiley">';elseif($n<20)if($n===17)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'huh.gif" alt="???" title="Huh?" class="smiley">';elseif($n===18)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'police.gif" alt="C:-)" title="Police" class="smiley">';else$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'afro.gif" alt="O0" title="Afro" class="smiley">';elseif($n===20)$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'angel.gif" alt="O:-)" title="Angel" class="smiley">';else$html.='<img src="'.htmlspecialchars($this->params['SMILEYS_PATH'],2).'azn.gif" alt="^-^" title="Azn" class="smiley">';}else$html.=htmlspecialchars($textContent,0);}else{$attributes+=['flash0'=>null,'flash1'=>null,'content'=>null];$html='';if(!empty($this->params['IS_IE']))$html.='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'.$attributes['flash0'].'" height="'.$attributes['flash1'].'"><param name="movie" value="'.$attributes['content'].'"><param name="play" value="true"><param name="loop" value="true"><param name="quality" value="high"><param name="AllowScriptAccess" value="never"><embed src="'.$attributes['content'].'" width="'.$attributes['flash0'].'" height="'.$attributes['flash1'].'" play="true" loop="true" quality="high" allowscriptaccess="never"><noembed><a href="'.$attributes['content'].'" target="_blank" class="new_win">'.str_replace('&quot;','"',$attributes['content']).'</a></noembed></object>';else$html.='<embed type="application/x-shockwave-flash" src="'.$attributes['content'].'" width="'.$attributes['flash0'].'" height="'.$attributes['flash1'].'" play="true" loop="true" quality="high" allowscriptaccess="never"><noembed><a href="'.$attributes['content'].'" target="_blank" class="new_win">'.str_replace('&quot;','"',$attributes['content']).'</a></noembed>';}elseif($qb<9)if($qb===6){$attributes+=['glow0'=>null,'glow1'=>null];$html='';if(!empty($this->params['IS_IE']))$html.='<table border="0" cellpadding="0" cellspacing="0" style="display: inline; vertical-align: middle; font: inherit;"><tr><td style="filter: Glow(color='.$attributes['glow0'].', strength='.$attributes['glow1'].'); font: inherit;">';else$html.='<span style="text-shadow: '.$attributes['glow0'].' 1px 1px 1px">';}elseif($qb===7){$attributes+=['src'=>null];$html='<img src="'.$attributes['src'].'"';if(isset($attributes['alt']))$html.=' alt="'.$attributes['alt'].'"';if(isset($attributes['height']))$html.=' height="'.$attributes['height'].'"';if(isset($attributes['width']))$html.=' width="'.$attributes['width'].'"';$html.=' class="bbc_img';if(isset($attributes['height'])||isset($attributes['width']))$html.=' resized';$html.='">';}else{$html='';if(isset($attributes['type']))$html.='<ul class="bbc_list" style="list-style-type: '.$attributes['type'].';">';else$html.='<ul class="bbc_list">';self::$attributes[]=$attributes;}elseif($qb===9){$attributes+=['me'=>null];$html='<div class="meaction">* '.str_replace('&quot;','"',$attributes['me']).' ';}elseif($qb===10){$attributes+=['link'=>null,'author'=>null];$html='<div class="quoteheader"><div class="topslice_quote">';if(!isset($attributes['author']))$html.=htmlspecialchars($this->params['L_QUOTE'],0);elseif(isset($attributes['date'])&&isset($attributes['link']))$html.='<a href="'.htmlspecialchars($this->params['SCRIPT_URL'],2).'?'.$attributes['link'].'">'.htmlspecialchars($this->params['L_QUOTE_FROM'],0).': '.str_replace('&quot;','"',$attributes['author']).' '.htmlspecialchars($this->params['L_SEARCH_ON'],0).' '.str_replace('&quot;','"',$attributes['date']).'</a>';else$html.=htmlspecialchars($this->params['L_QUOTE_FROM'],0).': '.str_replace('&quot;','"',$attributes['author']);$html.='</div></div><blockquote>';}else{$attributes+=['time'=>null];$html=str_replace('&quot;','"',$attributes['time']);}
