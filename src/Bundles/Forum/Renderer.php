...
class Renderer extends \s9e\TextFormatter\Renderer
{
	...
	private static $dynamic=['COLOR'=>['(^\\S*(?> (?!color=)[^=]+="[^"]*")*(?> color="([^"]+)")?.*)s','<span style="color:$1">'],'DAILYMOTION'=>['(^\\S*(?> (?!id=)[^=]+="[^"]*")*(?> id="([^"]+)")?.*)s','<iframe width="560" height="315" src="//www.dailymotion.com/embed/video/$1" allowfullscreen="" frameborder="0" scrolling="no"></iframe>'],'EMAIL'=>['(^\\S*(?> (?!email=)[^=]+="[^"]*")*(?> email="([^"]+)")?.*)s','<a href="mailto:$1">'],'FACEBOOK'=>['(^\\S*(?> (?!id=)[^=]+="[^"]*")*(?> id="([^"]+)")?.*)s','<iframe width="560" height="315" src="//s9e.github.io/iframe/facebook.min.html#$1" onload="var a=Math.random();window.addEventListener(\'message\',function(b){if(b.data.id==a)style.height=b.data.height+\'px\'});contentWindow.postMessage(\'s9e:\'+a,src.substr(0,src.indexOf(\'/\',8)))" allowfullscreen="" frameborder="0" scrolling="no"></iframe>'],'FONT'=>['(^\\S*(?> (?!font=)[^=]+="[^"]*")*(?> font="([^"]+)")?.*)s','<span style="font-family:$1">'],'INDIEGOGO'=>['(^\\S*(?> (?!id=)[^=]+="[^"]*")*(?> id="([^"]+)")?.*)s','<iframe width="224" height="486" src="//www.indiegogo.com/project/$1/widget" allowfullscreen="" frameborder="0" scrolling="no"></iframe>'],'INSTAGRAM'=>['(^\\S*(?> (?!id=)[^=]+="[^"]*")*(?> id="([^"]+)")?.*)s','<iframe width="612" height="710" src="//instagram.com/p/$1/embed/" allowfullscreen="" frameborder="0" scrolling="no"></iframe>'],'LIVELEAK'=>['(^\\S*(?> (?!id=)[^=]+="[^"]*")*(?> id="([^"]+)")?.*)s','<iframe width="640" height="360" src="http://www.liveleak.com/ll_embed?i=$1" allowfullscreen="" frameborder="0" scrolling="no"></iframe>'],'SIZE'=>['(^\\S*(?> (?!size=)[^=]+="[^"]*")*(?> size="([^"]+)")?.*)s','<span style="font-size:$1px">'],'URL'=>['(^\\S*(?> (?!(?>title|url)=)[^=]+="[^"]*")*( title="[^"]+")?(?> (?!(?>title|url)=)[^=]+="[^"]*")*(?> url="([^"]+)")?.*)s','<a href="$2"$1>'],'VIMEO'=>['(^\\S*(?> (?!id=)[^=]+="[^"]*")*(?> id="([^"]+)")?.*)s','<iframe width="560" height="315" src="//player.vimeo.com/video/$1" allowfullscreen="" frameborder="0" scrolling="no"></iframe>'],'VINE'=>['(^\\S*(?> (?!id=)[^=]+="[^"]*")*(?> id="([^"]+)")?.*)s','<iframe width="480" height="480" src="https://vine.co/v/$1/embed/simple" allowfullscreen="" frameborder="0" scrolling="no"></iframe><script async="" src="//platform.vine.co/static/scripts/embed.js" charset="utf-8"></script>'],'WSHH'=>['(^\\S*(?> (?!id=)[^=]+="[^"]*")*(?> id="([^"]+)")?.*)s','<iframe width="640" height="360" src="//www.worldstarhiphop.com/embed/$1" allowfullscreen="" frameborder="0" scrolling="no"></iframe>']];
	...
	protected function quick($m)
	{
		...
		if ($id[0] !== '/')
		{
			...
			foreach ($matches[1] as $i => $attrName)
				$attributes[$attrName] = htmlspecialchars_decode($matches[2][$i]);
		}

		$qb = self::$quickBranches[$id];
		if($qb<6)if($qb<3)if($qb===0){$attributes=array_pop(self::$attributes);$html='';if(!isset($attributes['type']))$html.='</ul>';elseif(strpos('upperlowerdecim',substr($attributes['type'],0,5))!==false)$html.='</ol>';else$html.='</ul>';}elseif($qb===1){$attributes+=['track_num'=>null,'track_id'=>null];$html='<iframe width="400" height="400" allowfullscreen="" frameborder="0" scrolling="no" src="//bandcamp.com/EmbeddedPlayer/size=large/minimal=true/';if(isset($attributes['album_id'])){$html.='album='.htmlspecialchars($attributes['album_id'],2);if(isset($attributes['track_num']))$html.='/t='.htmlspecialchars($attributes['track_num'],2);}else$html.='track='.htmlspecialchars($attributes['track_id'],2);$html.='"></iframe>';}else{$html='';if(isset(self::$bt9C97241C[$textContent])){$n=self::$bt9C97241C[$textContent];if($n<10)if($n<5)if($n<3)if($n===0)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/frown.png" alt=":(">';elseif($n===1)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/smile.png" alt=":)">';else$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/frown.png" alt=":-(">';elseif($n===3)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/smile.png" alt=":-)">';else$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/kiss.png" alt=":-*">';elseif($n<8)if($n===5)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/confused.png" alt=":-?">';elseif($n===6)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/grin.png" alt=":-D">';else$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/razz.png" alt=":-P">';elseif($n===8)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/razz.png" alt=":-p">';else$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/neutral.png" alt=":-|">';elseif($n<15)if($n<13)if($n===10)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/confused.png" alt=":?">';elseif($n===11)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/grin.png" alt=":D">';else$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/razz.png" alt=":P">';elseif($n===13)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/laugh.png" alt=":lol:">';else$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/shock.png" alt=":o">';elseif($n===15)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/razz.png" alt=":p">';elseif($n===16)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/neutral.png" alt=":|">';elseif($n===17)$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/wink.png" alt=";)">';else$html.='<img src="'.htmlspecialchars($this->params['EMOTICONS_PATH'],2).'/wink.png" alt=";-)">';}else$html.=htmlspecialchars($textContent,0);}elseif($qb===3){$attributes+=['playlistid'=>null,'songid'=>null];$html='<object type="application/x-shockwave-flash" typemustmatch="" width="400" height="'.htmlspecialchars((isset($attributes['songid'])?40:400),2).'" data="//grooveshark.com/'.htmlspecialchars((isset($attributes['songid'])?'songW':'w'),2).'idget.swf"><param name="allowfullscreen" value="true"><param name="flashvars" value="playlistID='.htmlspecialchars($attributes['playlistid'],2).'&amp;songID='.htmlspecialchars($attributes['songid'],2).'"><embed type="application/x-shockwave-flash" src="//grooveshark.com/'.htmlspecialchars((isset($attributes['songid'])?'songW':'w'),2).'idget.swf" width="400" height="'.htmlspecialchars((isset($attributes['songid'])?40:400),2).'" allowfullscreen="" flashvars="playlistID='.htmlspecialchars($attributes['playlistid'],2).'&amp;songID='.htmlspecialchars($attributes['songid'],2).'"></object>';}elseif($qb===4){$attributes+=['id'=>null];$html='';if(isset($attributes['video']))$html.='<iframe width="480" height="360" src="//www.kickstarter.com/projects/'.htmlspecialchars($attributes['id'],2).'/widget/video.html" allowfullscreen="" frameborder="0" scrolling="no"></iframe>';else$html.='<iframe width="220" height="380" src="//www.kickstarter.com/projects/'.htmlspecialchars($attributes['id'],2).'/widget/card.html" allowfullscreen="" frameborder="0" scrolling="no"></iframe>';}else{$attributes+=['type'=>null];$html='';if(!isset($attributes['type']))$html.='<ul>';elseif(strpos('upperlowerdecim',substr($attributes['type'],0,5))!==false)$html.='<ol style="list-style-type:'.htmlspecialchars($attributes['type'],2).'">';else$html.='<ul style="list-style-type:'.htmlspecialchars($attributes['type'],2).'">';self::$attributes[]=$attributes;}elseif($qb<9)if($qb===6){$html='<blockquote';if(!isset($attributes['author']))$html.=' class="uncited"';$html.='><div>';if(isset($attributes['author']))$html.='<cite>'.htmlspecialchars($attributes['author'],0).' '.htmlspecialchars($this->params['L_WROTE'],0).'</cite>';}elseif($qb===7){$attributes+=['playlist_id'=>null,'track_id'=>null,'id'=>null];$html='<iframe width="100%" height="166" style="max-width:900px" allowfullscreen="" frameborder="0" scrolling="no" src="https://w.soundcloud.com/player/?url=';if(isset($attributes['secret_token'])&&isset($attributes['playlist_id']))$html.='https://api.soundcloud.com/playlists/'.htmlspecialchars($attributes['playlist_id'],2).'&amp;secret_token='.htmlspecialchars($attributes['secret_token'],2);elseif(isset($attributes['secret_token'])&&isset($attributes['track_id']))$html.='https://api.soundcloud.com/tracks/'.htmlspecialchars($attributes['track_id'],2).'&amp;secret_token='.htmlspecialchars($attributes['secret_token'],2);else{if((strpos($attributes['id'],'://')===false))$html.='https://soundcloud.com/';$html.=htmlspecialchars($attributes['id'],2);if(isset($attributes['secret_token']))$html.='&amp;secret_token='.htmlspecialchars($attributes['secret_token'],2);}$html.='"></iframe>';}else{$attributes+=['title'=>null];$html='<div class="spoiler"><div class="spoiler-header"><button onclick="var c=this.parentNode.nextSibling.style,s=this.firstChild.style,h=this.lastChild.style;\'\'!=c.display?(c.display=h.display=\'\',s.display=\'none\'):(c.display=h.display=\'none\',s.display=\'\')"><span>'.htmlspecialchars($this->params['L_SHOW'],0).'</span><span style="display:none">'.htmlspecialchars($this->params['L_HIDE'],0).'</span></button><span class="spoiler-title">'.htmlspecialchars($this->params['L_SPOILER'],0).' '.htmlspecialchars($attributes['title'],0).'</span></div><div class="spoiler-content" style="display:none">';}elseif($qb===9){$attributes+=['channel'=>null];$html='<object type="application/x-shockwave-flash" typemustmatch="" width="620" height="378" data="//www.twitch.tv/widgets/';if(isset($attributes['archive_id'])||isset($attributes['chapter_id']))$html.='arch';else$html.='l';$html.='ive_embed_player.swf"><param name="allowfullscreen" value="true"><param name="flashvars" value="channel='.htmlspecialchars($attributes['channel'],2);if(isset($attributes['archive_id']))$html.='&amp;archive_id='.htmlspecialchars($attributes['archive_id'],2);if(isset($attributes['chapter_id']))$html.='&amp;chapter_id='.htmlspecialchars($attributes['chapter_id'],2);$html.='&amp;auto_play=false"><embed type="application/x-shockwave-flash" width="620" height="378" allowfullscreen="" src="//www.twitch.tv/widgets/';if(isset($attributes['archive_id'])||isset($attributes['chapter_id']))$html.='arch';else$html.='l';$html.='ive_embed_player.swf" flashvars="channel='.htmlspecialchars($attributes['channel'],2);if(isset($attributes['archive_id']))$html.='&amp;archive_id='.htmlspecialchars($attributes['archive_id'],2);if(isset($attributes['chapter_id']))$html.='&amp;chapter_id='.htmlspecialchars($attributes['chapter_id'],2);$html.='&amp;auto_play=false"></object>';}else{$attributes+=['id'=>null,'m'=>null,'s'=>null];$html='<iframe width="560" height="315" allowfullscreen="" frameborder="0" scrolling="no" src="//www.youtube.com/embed/'.htmlspecialchars($attributes['id'],2).'?controls=2';if(isset($attributes['list']))$html.='&amp;list='.htmlspecialchars($attributes['list'],2);if(isset($attributes['t'])||isset($attributes['m'])){$html.='&amp;start=';if(isset($attributes['t']))$html.=htmlspecialchars($attributes['t'],2);elseif(isset($attributes['h']))$html.=htmlspecialchars($attributes['h']*3600+$attributes['m']*60+$attributes['s'],2);else$html.=htmlspecialchars($attributes['m']*60+$attributes['s'],2);}$html.='"></iframe>';}
